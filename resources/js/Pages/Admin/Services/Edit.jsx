...
export default function Edit({ service }) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        benefits: service.benefits?.map((b) => ({
            ...b,
            points: b.points || (b.description ? [b.description] : [""]),
        ...
        expect_results: service.expect_results || [
        ...
        // removed
        range_features: service.range_features || [{ feature_text: "", min_price: 0, max_price: "" }],
        _method: "PUT",
    });
    ...
    // Range Features Helpers
    const addRangeFeature = () => {
        setData("range_features", [
            ...data.range_features,
            { feature_text: "", min_price: 0, max_price: "" },
        ]);
    };

    const removeRangeFeature = (index) => {
        const newFeatures = data.range_features.filter((_, i) => i !== index);
        setData("range_features", newFeatures);
    };

    const updateRangeFeature = (index, field, value) => {
        const newFeatures = [...data.range_features];
            ...
            <div className="space-y-4 max-w-[1200px] mx-auto pb-20">
                ...
                >
                    ...
                    <div className="col-span-12 lg:col-span-8 space-y-5">
                        ...
                        {/* Dynamic Range Features Section */}
                        <div className="bg-white rounded-[12px] border border-[#e3e4e8] shadow-sm p-8">
                            <div className="flex items-center justify-between mb-6">
                                <div className="flex items-center gap-3">
                                    <Zap
                                    ...
                                    />
                                    <h2 className="text-[18px] font-bold text-[#2f3344]">
                                        Dynamic Range Features (Campaign Tiers)
                                    </h2>
                                </div>
                                <button
                                    type="button"
                                    onClick={addRangeFeature}
                                    className="text-orange-500 text-[13px] font-bold hover:underline flex items-center gap-1"
                                >
                                    <Plus size={16} /> Add Range Feature
                                </button>
                            </div>

                            <div className="space-y-4">
                                {data.range_features.map((feature, index) => (
                                    <div
                                        key={index}
                                        className="p-4 bg-orange-50/20 rounded-xl border border-orange-100/50 space-y-3"
                                    >
                                        <div className="flex gap-4">
                                            <div className="flex-1 space-y-1">
                                                ...
                                                <input
                                                    ...
                                                    placeholder="e.g., 20 Web 2.0 Blogs"
                                                    className="w-full h-[40px] px-3 bg-white border border-[#e3e4e8] rounded-lg outline-none text-[14px]"
                                                />
                                            </div>
                                            ...
                                            <div className="pt-6">
                                                <button
                                                    type="button"
                                                    onClick={() => removeRangeFeature(index)}
                                                    className="text-red-400 hover:text-red-600 transition-colors"
                                                >
                                                    <Trash2 size={20} />
                                                </button>
                            ...
                            <p className="mt-4 text-[12px] text-[#727586]">
                                * Features will automatically show on the frontend for campaigns within the specified price range.
                            </p>
                        </div>
